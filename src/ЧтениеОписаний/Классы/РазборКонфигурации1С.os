...
Перем СтруктураКаталогов Экспорт;
...
Процедура ПрочитатьСодержимоеМодуля(СтрокаМодуль) Экспорт
	...
	Если СтрокаМодуль.ТипМодуля = ТипыМодуля.ОбщийМодуль Тогда
		...
		Если СтрокаМодуль.Родитель.Описание = Неопределено Тогда
			...
			ПрочитатьОписаниеОбъекта(СтрокаМодуль.Родитель);
...
Функция ПрочитатьОписаниеКонфигурации()
	...
	ДанныеКонфигурации = ПрочитатьФайлОписанияОбъекта(ФайлКонфигурации, ТипКорневогоЭлемента);
	
	// добавим руками объект "Configuration" для модулей приложения
	НовСтрока = ОписаниеКонфигурации.ОбъектыКонфигурации.Добавить();
	НовСтрока.Тип = ТипКорневогоЭлемента;
...
Процедура ПриСозданииОбъекта(КаталогИсходников, пТипКорневогоЭлемента = Неопределено)
	
	Если пТипКорневогоЭлемента = Неопределено Тогда
		
		ТипКорневогоЭлемента = ТипыОбъектовКонфигурации.ИмяТипаКонфигурации();
	...
	Если СтруктураКаталогов.ФорматВыгрузки() = ФорматыВыгрузки.EDT Тогда
